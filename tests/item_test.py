...
class ItemTest(TestBase):
    ...
    def test_get_possible_tags_without_specifics_with_undefined(self) -> None:
        item = Item(self.api, self.category, 1)
    ...
    def test_get_possible_tags_without_undefined(self) -> None:
        item = Item(self.api, self.category, 1)
        tags = item.get_possible_tags(add_undefined=False)
        self.assertEquals({self.category.name_path[-1]}, tags)

    def test_get_possible_tags_with_one_specific(self) -> None:
    ...
    def test_get_possible_tags_with_multiple_specifics(self) -> None:
        self.api.get_item = partial(
            create_item_dict, specifics={
                'NameValueList': [
                    {'Name': 'stil', 'Value': 'blah'},
    ...
    def test_get_possible_tags_with_ignored_specific(self) -> None:
        self.api.get_item = partial(
            create_item_dict, specifics={
                'NameValueList': [
    ...
    def test_download_images_jpg(self) -> None:
    ...
    def test_download_images_nonexistent(self) -> None:
